...
def build_inv_tx_select(db: Session) -> str:
    """inventory_tx 컬럼 존재 여부에 따라 안전한 SELECT 목록 구성."""
    parts: List[str] = []

    parts.append("id::text as id" if has_column(db,"inventory_tx","id") else "NULL::text as id")
    parts.append("ingredient_id::text as ingredient_id" if has_column(db,"inventory_tx","ingredient_id") else "NULL::text as ingredient_id")
    parts.append("location_id::text as location_id" if has_column(db,"inventory_tx","location_id") else "NULL::text as location_id")
    ...
    parts.append("qty_delta::float as qty_delta" if has_column(db,"inventory_tx","qty_delta") else "0.0::float as qty_delta")
    parts.append("ref_table as ref_table" if has_column(db,"inventory_tx","ref_table") else "NULL::text as ref_table")
    parts.append("ref_id::text as ref_id" if has_column(db,"inventory_tx","ref_id") else "NULL::text as ref_id")
    parts.append("note as note" if has_column(db,"inventory_tx","note") else "NULL::text as note")
    parts.append("created_at as created_at" if has_column(db,"inventory_tx","created_at") else "now() as created_at")

    return ",\n       ".join(parts)

def build_where(db: Session, ingredient_id: Optional[str], location_id: Optional[str], since: Optional[str], params: Dict[str, Any]) -> str:
    conds: List[str] = []
    ...
    rows = db.execute(
        text(f"""
            SELECT ingredient_id::text,
                   location_id::text,
                   COALESCE(qty_on_hand,0)::float AS qty_on_hand
            FROM inv_stock
            {where}
            ORDER BY ingredient_id, location_id
        """),
        params
    ).mappings().all()
...
def list_inventory_tx(
    ...
    try:
        ...
        # 실패 시에도 프론트가 원인 파악할 수 있게 메시지 노출

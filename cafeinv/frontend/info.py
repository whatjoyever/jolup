...
# 1. 페이지 레이아웃을 'wide'로 설정하여 전체 너비를 사용하게 합니다.
#    이 코드는 스크립트에서 st 요소들을 사용하기 전에 가장 먼저 호출***
st.set_page_config(layout="wide")

# 2. Streamlit의 기본 여백(padding)을 제거하기 위한 CSS를 주입
st.markdown("""
<style>
...
# 탭 구성
# ------------------------------
category_tab, product_tab, partner_tab, admin_tab = st.tabs(["카테고리 등록", "품목 등록", "거래처 관리", "관리자 등록"])

# 세션 상태 초기화
if "categories" not in st.session_state:
    st.session_state.categories = []  # [{"code": str, "name": str}]
if "category_selected" not in st.session_state:
    st.session_state.category_selected = set()  # indices selected for delete
if "category_edit_mode" not in st.session_state:
    st.session_state.category_edit_mode = False  # 수정 모드 플래그
if "products" not in st.session_state:
    st.session_state.products = []  # [{"code","category","name","unit","status"}]
...
if "product_edit_mode" not in st.session_state:
    st.session_state.product_edit_mode = False  # 수정 모드 플래그
if "partners" not in st.session_state:
    st.session_state.partners = []  # [{"code","name","business_number","representative","address"}]
...
if "partner_edit_mode" not in st.session_state:
    st.session_state.partner_edit_mode = False  # 수정 모드 플래그
if "admins" not in st.session_state:
    st.session_state.admins = []  # [{"emp_no","name","gender","email","phone","position","management_type","status"}]
if "admin_selected" not in st.session_state:
    st.session_state.admin_selected = set()
if "admin_edit_mode" not in st.session_state:
    st.session_state.admin_edit_mode = False  # 수정 모드 플래그
...
with category_tab:
    ...
    with st.form("category_list_form"):
        if len(st.session_state.categories) == 0:
            ...
            # 등록된 카테고리들을 표시
            for idx, row in enumerate(st.session_state.categories):
                # 각 카테고리를 등록 칸과 똑같은 스타일로 표시
                cat_col1, cat_col2, cat_col3 = st.columns([2, 3, 1])
                with cat_col1:
                    st.caption("코드번호")
                    st.text_input("코드번호", value=row["code"], key=f"cat_code_{idx}", disabled=not st.session_state.category_edit_mode, label_visibility="collapsed")
                with cat_col2:
                    st.caption("카테고리명")
                    st.text_input("카테고리명", value=row["name"], key=f"cat_name_{idx}", disabled=not st.session_state.category_edit_mode, label_visibility="collapsed")
                with cat_col3:
                    st.caption("\u00A0")
                    st.markdown("<div style='height: 37px'></div>", unsafe_allow_html=True)
                    # 체크박스만 표시
...
with product_tab:
    ...
    with st.form("product_form", clear_on_submit=True):
        ...
        with r1c1:
            st.caption("코드번호")
            pr_code = st.text_input("코드번호", value=default_code, key="prod_code_input", label_visibility="collapsed", placeholder="pr_001")
        with r1c2:
            st.caption("카테고리명")
        ...
        with r1c3:
            st.caption("품목 명")
            pr_name = st.text_input("품목 명", value=default_name, key="prod_name_input", label_visibility="collapsed", placeholder="디카페인 원두")
        ...
        with r2c3:
            st.caption("안전재고")
            pr_safety = st.number_input("안전재고", min_value=0, step=1, value=0, key="prod_safety_input", label_visibility="collapsed")
        with r2c4:
            st.caption("\u00A0")
        ...
        if pr_submitted:
            ...
            if not code or not name:
                ...
                if any(p["code"] == code for p in st.session_state.products):
                    ...
                    st.session_state.products.append({"code": code, "category": cat, "name": name, "unit": unit, "status": pr_status, "safety": int(pr_safety)})
                    st.success("등록되었습니다.")
                    st.rerun()
    ...
    with st.form("product_list_form"):
        ...
        if len(st.session_state.products) == 0:
            ...
            for filtered_idx, pr in enumerate(filtered_products):
                ...
                with c2:
                    st.text_input("코드번호", value=pr["code"], key=f"prod_code_{original_idx}", disabled=not st.session_state.product_edit_mode, label_visibility="collapsed")
                with c3:
                    st.text_input("품목명", value=pr["name"], key=f"prod_name_{original_idx}", disabled=not st.session_state.product_edit_mode, label_visibility="collapsed")
                with c4:
                    if st.session_state.product_edit_mode:
                        unit_options = ["병", "박스", "kg", "갯수", "기타"]
                        ...
                        st.text_input("단위", value=pr["unit"], key=f"prod_unit_{original_idx}", disabled=True, label_visibility="collapsed")
                with c5:
                    if st.session_state.product_edit_mode:
                        st.selectbox(
                            "상태",
                        ...
                        st.text_input("상태", value=pr["status"], key=f"prod_status_{original_idx}", disabled=True, label_visibility="collapsed")
                with c6:
                    if st.session_state.product_edit_mode:
                        st.number_input("안전재고", min_value=0, step=1, value=int(pr.get("safety", 0)), key=f"prod_safety_{original_idx}", label_visibility="collapsed")
                    else:
                        st.text_input("안전재고", value=str(pr.get("safety", 0)), key=f"prod_safety_{original_idx}", disabled=True, label_visibility="collapsed")
                with c7:
                    st.write("")  # 작업 컬럼은 비움
    ...
    with st.form("partner_list_form"):
        ...
        if len(st.session_state.partners) == 0:
            ...
            for idx, partner in enumerate(st.session_state.partners):
                ...
                with c1:
                    st.text_input("거래처 코드", value=partner["code"], key=f"partner_code_{idx}", disabled=not st.session_state.partner_edit_mode, label_visibility="collapsed")
                with c2:
                    st.text_input("거래처명", value=partner["name"], key=f"partner_name_{idx}", disabled=not st.session_state.partner_edit_mode, label_visibility="collapsed")
                with c3:
                    st.text_input("사업자번호", value=partner.get("business_number", ""), key=f"partner_bus_{idx}", disabled=not st.session_state.partner_edit_mode, label_visibility="collapsed")
                with c4:
                    st.text_input("대표자", value=partner.get("representative", ""), key=f"partner_rep_{idx}", disabled=not st.session_state.partner_edit_mode, label_visibility="collapsed")
                with c5:
                    st.text_input("주소", value=partner.get("address", ""), key=f"partner_addr_{idx}", disabled=not st.session_state.partner_edit_mode, label_visibility="collapsed")
                with c6:
                    checked = st.checkbox("", key=f"partner_sel_{idx}")
                    if checked:
                        st.session_state.partner_selected.add(idx)
                    else:
                        st.session_state.partner_selected.discard(idx)
        ...
        if len(st.session_state.admins) == 0:
            ...
            for idx, admin in enumerate(st.session_state.admins):
                ...
                with c1:
                    ...
                    if checked:
                        st.session_state.admin_selected.add(idx)
                    else:
                        st.session_state.admin_selected.discard(idx)
                with c2:
                    st.text_input("사번번호", value=admin["emp_no"], key=f"admin_emp_no_{idx}", disabled=not st.session_state.admin_edit_mode, label_visibility="collapsed")
                with c3:
                    st.text_input("이름", value=admin["name"], key=f"admin_name_{idx}", disabled=not st.session_state.admin_edit_mode, label_visibility="collapsed")
                with c4:
                    if st.session_state.admin_edit_mode:
                        st.selectbox("성별", options=["남", "여"], index=(0 if admin["gender"] == "남" else 1), key=f"admin_gender_{idx}", label_visibility="collapsed")
                    else:
                        st.text_input("성별", value=admin["gender"], key=f"admin_gender_{idx}", disabled=True, label_visibility="collapsed")
                with c5:
                    if st.session_state.admin_edit_mode:
                        ...
                        st.text_input("연락처", value=f"{admin['email']} / {admin['phone']}", key=f"admin_contact_{idx}", disabled=True, label_visibility="collapsed")
                with c6:
                    if st.session_state.admin_edit_mode:
                        ...
                        st.selectbox("직급", options=position_options, index=pos_index, key=f"admin_position_{idx}", label_visibility="collapsed")
                    else:
                        st.text_input("직급", value=admin["position"], key=f"admin_position_{idx}", disabled=True, label_visibility="collapsed")
                with c7:
                    if st.session_state.admin_edit_mode:
                        ...
                        st.selectbox("관리 종류", options=mgmt_options, index=mgmt_index, key=f"admin_mgmt_type_{idx}", label_visibility="collapsed")
                    else:
                        st.text_input("관리 종류", value=admin["management_type"], key=f"admin_mgmt_type_{idx}", disabled=True, label_visibility="collapsed")
                with c8:
                    if st.session_state.admin_edit_mode:
                        status_options = ["재직", "퇴사", "휴직"]
                        status_index = status_options.index(admin["status"]) if admin["status"] in status_options else 0
                        st.selectbox("재직현황", options=status_options, index=status_index, key=f"admin_status_{idx}", label_visibility="collapsed")
                    else:
                        st.text_input("재직현황", value=admin["status"], key=f"admin_status_{idx}", disabled=True, label_visibility="collapsed")
                with c9:
                    st.write("")  # 수정 컬럼은 비움
